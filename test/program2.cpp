...
int main(int argc, char* argv[])
{
	...
	glBufferData(GL_ARRAY_BUFFER, ud->ver.size() * sizeof(float), ud->ver.data(), GL_STATIC_DRAW);
	glEnableVertexAttribArray(0);
	glVertexAttribPointer(0, 2, GL_FLOAT, false, 2 * sizeof(float), nullptr);
	glBindBuffer(GL_ARRAY_BUFFER, vbo[1]);
	glBufferData(GL_ARRAY_BUFFER, ud->col.size() * sizeof(float), ud->col.data(), GL_STATIC_DRAW);
